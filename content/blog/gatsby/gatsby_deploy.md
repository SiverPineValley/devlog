...
gatsby용으로 생성한 repository 선택 후, 빌드 파라미터를 아래와 같이 입력해야 한다. 특히 환경변수가 중요한데, 내가 선택한 [gatsby-starter-bee](https://github.com/JaeYeopHan/gatsby-starter-bee)의 경우 node-sass를 사용하는데, 16 버전 이상의 Node에서는 정상적으로 빌드가 불가능하다. Stable 최신 버전인 14.17.6을 입력하였다. 다 입력했으면, 아래에서 `Deploy Site` 버튼 클릭.
...
정상적으로 빌드되었다면 아래와 같은 문구로 끝나게 된다.
이제 빌드된 사이트로 접속하면 끝~ 😎😎😎
